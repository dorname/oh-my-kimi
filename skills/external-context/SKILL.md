...
- Use `Agent(subagent_type="coder", description="Lookup official docs", ...)` for official doc lookup
...
```
Agent(subagent_type="coder", description="Lookup Redis docs", prompt="You are the Document Specialist agent. Find the official Redis Python client docs. What are the breaking changes in redis-py 5.0?")
Agent(subagent_type="coder", description="Lookup FastAPI docs", prompt="You are the Document Specialist agent. Check FastAPI docs for background tasks. What's the recommended pattern?")
```
