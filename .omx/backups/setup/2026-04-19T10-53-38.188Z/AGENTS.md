...
- **Lightweight analysis** (OMK haiku tier): Keep prompts concise, focus on pattern matching and lookups.
- **Standard work** (OMK sonnet tier): Use balanced detail, include examples, request structured output.
- **Deep reasoning** (OMK opus tier): Provide full context, ask for step-by-step reasoning, request trade-off analysis.
